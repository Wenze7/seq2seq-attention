...
def lstm_encoder(sequence, lstm, seq_lens=None, init_states=None, embedding=None):
    ...
    if seq_lens:
        ...
        emb_sequence = _reorder_sequence(emb_sequence, sort_ind,
                                        lstm.batch_first)

    if init_states is None:
        device = sequence.device
        init_states = _init_lstm_states(lstm, batch_size, device)
    else:
    ...
    if seq_lens:
        ...
        lstm_out = _reorder_sequence(lstm_out, reorder_ind, lstm.batch_first)
        final_states = _reorder_lstm_states(final_states, reorder_ind)
    else:
...
## LSTM helpers
def _init_lstm_states(lstm, batch_size, device):
...
def _reorder_sequence(sequence_emb, order, batch_first=False):
    """
...
def _reorder_lstm_states(lstm_states, order):
    """
