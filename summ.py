...
class Seq2SeqSumm(nn.Module):
    def __init__(self, vocab_size, emb_dim,
        ...
        self._decoder = AttentionalDecoder(
            self._embedding, self._dec_lstm, self._attn_wq, 
            self._attention, self._projection
        )
...
class AttentionalDecoder(object):
    def __init__(self, embedding, lstm, attn_w, attention, projection):
        super().__init__()
    ...
    def _step(self, tok, states, attention):
        ...
        # compute attention of each step
        context, score = self._attention(query, attention, attention, attn_mask)
